...
KanDo is distributed as a `.webhapp` referenced by the
...
(KanDo 0.17.x) do not carry over, and a group has to move across together — a
0.7 agent cannot see 0.6 peers at all. Use **Settings → Import/Export Boards**
to carry board contents over.
...
over every zome named in `dnas/kando/workdir/dna.yaml` (`wasm-opt -Oz
...
The script verifies `workdir/kando.happ` against `.happ-sha256` and
...
3. The [`release-webhapp`](.github/workflows/release-webhapp.yaml) workflow then:
   - downloads the frozen happ from `happ-v<dnaVersion>` and checks its sha256
     against `.happ-sha256`,
   - builds the UI and packs `kando.webhapp` (no `--recursive`, so the happ
   ...
   - publishes a **prerelease** GitHub release with `kando.webhapp` attached.
...
Add a new `versions[]` entry for `kando` in the 0.16 curation list with the
new `version`, the release's `kando.webhapp` `url`, and these hashes. Because
