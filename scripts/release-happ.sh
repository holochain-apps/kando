...
#   1. `workdir/kando.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/kando.happ"
else
  happ="workdir/kando.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical kando.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download kando.happ from $HAPP_TAG."
